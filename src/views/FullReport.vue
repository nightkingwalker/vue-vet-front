<template>
  <div id="printable" class="px-4 lg:!text-[14px]">
    ...
    <Card>
      <template #title>
        <div class="flex justify-between items-center">
          ...
          <div>
            <Button icon="pi pi-print" @click="printComponent" class="p-button p-button-icon-only p-button-rounded mx-2"
              v-tooltip="$t('pet_details.print')" />
            <Button class="p-button p-button-icon-only p-button-rounded" icon="pi pi-microchip-ai"
              @click="generateAISummary" v-tooltip="`AI Assisted Report`" />
      ...
      <template #content>
        ...
        <Panel :header="$t('case_history.steps.general_health')" toggleable
          ...
          <p>
            <strong>{{ $t("case_history.fields.previous_diseases_in_other_animals") }}:</strong>
        ...
        <Panel :header="$t('case_history.steps.reproductive_history')" toggleable
          v-for="(history, index) in medicalRecord.reproductive_history" :key="index"
        ...
        <Panel :header="$t('case_history.steps.diet_water')" toggleable
        ...
        <Divider :align="!isRtl ? `left` : `right`" type="solid" id="XYZ">
          <span class="lg:!text-[14px] text-xl font-bold flex gap-2">
            <svg class="w-[24px] !text-[var(--p-dialog-color)] dark:!text-[var(--p-primary-contrast-color)]"
              fill="currentColor" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 59.003 59.003" xml:space="preserve">
	...
	S54.552,35.123,54,35.123z" />
        ...
        <Panel :header="$t('case_history.steps.gastrointestinal')" toggleable
          v-for="(symptom, index) in medicalRecord.gastrointestinal_symptom" :key="index"
          class="border rounded-lg shadow p-4">
          <p>
        ...
        <Panel :header="$t('case_history.steps.neurological')" toggleable
          v-for="(symptom, index) in medicalRecord.neurological_symptom" :key="index"
          class="border rounded-lg shadow p-4">
          <p>
          ...
          <p>
            <strong>{{ $t("case_history.fields.related_to_eating_or_activity") }}:</strong>
        ...
        <Panel :header="$t('case_history.steps.respiratory')" toggleable
          v-for="(symptom, index) in medicalRecord.respiratory_symptom" :key="index"
          class="border rounded-lg shadow p-4">
          <p>
	...
	s0.447,1,1,1H12z M44,24.178h-6c0.553,0,1-0.447,1-1s-0.447-1-1-1h6c0.553,0,1,0.447,1,1S44.553,24.178,44,24.178z" />
        ...
        <Panel :header="$t('case_history.steps.urinary')" toggleable
          v-for="(symptom, index) in medicalRecord.urinary_symptom" :key="index" class="border rounded-lg shadow p-4">
          <p>
        ...
        <Divider :align="!isRtl ? `left` : `right`" type="solid" id="vaccines">
          <span class="lg:!text-[14px] text-xl font-bold flex gap-2">
            <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 122.88 117.56"
        ...
        <Panel :header="$t('case_history.additional.vaccination_history')" toggleable
          v-for="(vaccine, index) in medicalRecord.vaccination_history" :key="index"
          class="border rounded-lg shadow p-4">
          <p>
        ...
        <Panel :header="$t('case_history.additional.medication_history')" toggleable
          v-for="(medication, index) in medicalRecord.medication_history" :key="index"
          class="border rounded-lg shadow p-4">
          <p>
            <strong>{{ $t("case_history.fields.medication_name") }}:</strong>
        ...
        <Panel :header="$t('case_history.additional.antiparasitic_treatment')" toggleable
          v-for="(treatment, index) in medicalRecord.antiparasitic_treatment" :key="index"
          class="border rounded-lg shadow p-4">
          <p>
        ...
        <!-- Additional Notes -->
        <Divider :align="!isRtl ? `left` : `right`" type="solid" id="XYZ">
          <span class="lg:!text-[14px] text-xl font-bold"><i class="fa-solid fa-note-sticky"></i>
            {{ $t("medical_examination.sections.notes") }}</span>
        </Divider>
        <Panel :header="$t('medical_examination.sections.notes')" toggleable
          v-for="(note, index) in medicalRecord.additional_note" :key="index" class="border rounded-lg shadow p-4">
          <p>
            <strong>{{ $t("case_history.fields.additional_notes") }}:</strong>
            {{ note.notes }}
          </p>
        ...
        <Divider :align="!isRtl ? `left` : `right`" type="solid" id="tests">
          <span class="lg:!text-[14px] text-xl font-bold"><i class="fa-solid fa-vials"></i> {{
            $t("pet_details.test_results") }}</span>
        </Divider>
        <Panel :header="$t('pet_details.test_results')" toggleable v-for="(test, index) in medicalRecord.test_results"
          :key="index" class="border rounded-lg shadow p-4">
          <div class="flex flex-wrap w-full">
            ...
            <div class="w-2/3">
              ...
              <div v-for="(entry, index) in parseResults(test.result)" :key="index" class="flex gap-2">
        ...
        <Divider :align="!isRtl ? `left` : `right`" type="solid" id="images">
          <span class="lg:!text-[14px] text-xl font-bold"><i class="fa-solid fa-x-ray text-2xl"></i>
            {{ $t("pet_details.xray_ultrasound") }}
          </span>
        </Divider>
        <div class="flex flex-wrap w-full gap-4">
          <Panel :header="$t('pet_details.xray_ultrasound')" toggleable
            v-for="(image, index) in medicalRecord.medical_images" :key="index"
        ...
        <Divider :align="!isRtl ? `left` : `right`" type="solid" id="examination">
          <span class="lg:!text-[14px] text-xl font-bold"><i class="fa-solid fa-stethoscope"></i>
        ...
        <div v-for="(exam, index) in medicalRecord.medical_examinations" :key="index" class="grid">
          ...
          <div class="flex flex-wrap gap-4">
            ...
            <Panel :header="$t('medical_examination.sections.head_neck')" toggleable
            ...
            <!-- Vital Signs -->
            <Panel :header="$t('medical_examination.sections.vital_signs')" toggleable
              class="w-[30%] border rounded-lg shadow p-4">
              <template #header>
            ...
            <Panel :header="$t('medical_examination.sections.nervous_system')" toggleable
              class="w-[30%] border rounded-lg shadow p-4">
              <p>
            ...
            <Panel :header="$t('medical_examination.sections.skin_coat')" toggleable
              class="w-[30%] border rounded-lg shadow p-4">
              <p>
            ...
            <Panel :header="$t('medical_examination.sections.abdominal')" toggleable
              class="w-[30%] border rounded-lg shadow p-4">
              <p>
            ...
            <Panel :header="$t('medical_examination.sections.body_condition')" toggleable
              class="w-[30%] border rounded-lg shadow p-4">
              <p>
        ...
        <div class="flex flex-wrap w-full gap-4">
          <Panel :header="$t('treatments.title')" toggleable v-for="(treatment, index) in medicalRecord.treatments"
            :key="index" class="border rounded-lg shadow p-4 w-[48%]">
            <p>
              <strong>{{ $t("case_history.fields.medication_name") }}:</strong>
              {{ treatment.name }}
            </p>
...
const setupScrollTracking = () => {
  containerRef.value = document.querySelector(".p-dialog-content");
  if (!containerRef.value) return;

  const onScroll = () => { /* your logic */ };
...
const prepareAISummaryPayload = (medicalRecord) => {
  return {
    pet_name: props.pet_name,
    pet_id: medicalRecord.pet_id,
    visit_date: medicalRecord.visit_date,
    diagnosis: medicalRecord.diagnosis,
    current_symptoms: medicalRecord.current_symptom,
    general_health: medicalRecord.general_health,
    reproductive_history: medicalRecord.reproductive_history,
    diet_water_intake: medicalRecord.diet_water_intake,
    gastrointestinal_symptom: medicalRecord.gastrointestinal_symptom,
    neurological_symptom: medicalRecord.neurological_symptom,
    respiratory_symptom: medicalRecord.respiratory_symptom,
    urinary_symptom: medicalRecord.urinary_symptom,
    vaccination_history: medicalRecord.vaccination_history,
    medication_history: medicalRecord.medication_history,
    antiparasitic_treatment: medicalRecord.antiparasitic_treatment,
    test_results: medicalRecord.test_results.map(r => ({
      test_type: r.test_type,
      result: r.result
    })),
    medical_images: medicalRecord.medical_images.map(img => ({
      type: img.type,
      organ: img.organ,
      measurements: img.measurements,
      description: img.description,
      ref_number: img.ref_number
    })),
...
const generateAISummary = async () => {
  ...
  // try {
  const payload = prepareAISummaryPayload(medicalRecord.value);

  // const response = await axios.post('/api/openai/summary', payload);
  // const response = await axiosInstance.post(
  //   // `/medical-records/31/full`
  //   '/openai/summary', payload
  // );
  emit("requestAiSummary", payload, props.medical_record_id);

  //   // This endpoint should be on your Laravel backend
  //   // which securely calls OpenAI API
  //   console.log(response.data)
  //   aiSummary.value = response.data; // Store AI's suggested diagnosis & treatments
  // } catch (error) {
